...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div>
        ...
        <h1 class="text-2xl font-bold text-slate-800 tracking-tight">Tambah Pengguna Baru</h1>
        <p class="text-slate-500 text-sm mt-1">Buat akun akses baru untuk sistem.</p>
    </div>
        ...
        <form action="{{ route('users.store') }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="nama_lengkap" class="w-full px-4 py-2.5 bg-white border border-slate-300 rounded-xl text-sm focus:ring-2 focus:ring-indigo-100 focus:border-indigo-500 outline-none transition-all" required placeholder="Contoh: Budi Santoso">
                </div>
                <!-- Username -->
                <div>
                     <label class="block text-sm font-semibold text-slate-700 mb-2">Username</label>
                    <input type="text" name="username" class="w-full px-4 py-2.5 bg-white border border-slate-300 rounded-xl text-sm focus:ring-2 focus:ring-indigo-100 focus:border-indigo-500 outline-none transition-all font-mono" required placeholder="username_login">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                     ...
                     <div class="relative">
                        <input type="password" name="password" id="password" class="w-full pl-4 pr-10 py-2.5 bg-white border border-slate-300 rounded-xl text-sm focus:ring-2 focus:ring-indigo-100 focus:border-indigo-500 outline-none transition-all" required>
                ...
                <div>
                     ...
                     <div class="relative">
                        <select name="role" id="role" class="w-full px-4 py-2.5 bg-white border border-slate-300 rounded-xl text-sm focus:ring-2 focus:ring-indigo-100 focus:border-indigo-500 outline-none appearance-none transition-all text-slate-700" required>
                            ...
                            <!-- Jika butuh role lain, tambahkan di sini -->
                        </select>
             ...
             <div id="area_dusun" class="hidden">
                 ...
                 <div class="relative">
                    <select name="id_dusun" class="w-full px-4 py-2.5 bg-white border border-slate-300 rounded-xl text-sm focus:ring-2 focus:ring-indigo-100 focus:border-indigo-500 outline-none appearance-none transition-all text-slate-700">
                        ...
                        {{-- Pastikan variabel $dusunList dikirim dari controller --}}
                         @if(isset($dusunList))
                 ...
                 <p class="text-xs text-slate-500 mt-1">Hanya wajib dipilih jika role adalah Kepala Dusun.</p>
            </div>
